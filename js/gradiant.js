...
class GradientBackground {
  constructor(sceneManager) {
    ...
    this.uniforms = {
      ...
      uColor1: { value: new THREE.Vector3(0.40, 0.18, 0.38) },  // 채도 낮춘 자주빛 보라
      uColor2: { value: new THREE.Vector3(0.08, 0.08, 0.10) },  // 거의 검정
      uColor3: { value: new THREE.Vector3(0.35, 0.15, 0.30) },  // 채도 낮춘 자주
      uColor4: { value: new THREE.Vector3(0.12, 0.10, 0.14) },  // 어두운 보라빛 회색
      uColor5: { value: new THREE.Vector3(0.38, 0.16, 0.34) },  // 채도 낮춘 보라+자주
      uColor6: { value: new THREE.Vector3(0.06, 0.06, 0.08) },  // 검정
      uSpeed: { value: 1.2 },
      uIntensity: { value: 1.8 },
      uTouchTexture: { value: null },
      uGrainIntensity: { value: 0.08 },
      uZoom: { value: 1.0 },
      uDarkNavy: { value: new THREE.Vector3(0.08, 0.08, 0.09) },
...
document.body.insertBefore(this.renderer.domElement, document.body.firstChild);
  ...
  this.scene.background = new THREE.Color(0x080808);
    u.uDarkNavy.value.set(0.05, 0.04, 0.06); 
...
gsap.to(".anim", {
  ...
  y: 0,
  duration: 1.2,
  ease: "elastic.out(1.2, 1.2)",  // 쫀득한 느낌
  stagger: 0,  // 글자마다 순서대로 등장
  delay: 0.3
});
...
document.querySelectorAll('summary').forEach(summary => {
  summary.addEventListener('mouseenter', () => {
    summary.style.boxShadow = `
      0 0 0 2px rgba(242, 210, 255, 0.76),
      0 0 30px 6px rgba(212, 169, 253, 0.5),
      0 0 80px 20px rgba(209, 179, 255, 0.3)
    `;
    summary.style.transition = 'box-shadow 0.4s ease';
  });

  summary.addEventListener('mouseleave', () => {
    summary.style.boxShadow = `
      0 0 20px 4px rgba(99, 60, 92, 0.48),
      0 0 60px 10px rgba(180, 100, 220, 0.12),
...
gsap.to(".phrase_wrap p", {
  ...
  scrollTrigger: {
    trigger: ".phrase_wrap",
    start: "top 80%", // 뷰포트 80% 지점에 요소 top이 닿을 때
